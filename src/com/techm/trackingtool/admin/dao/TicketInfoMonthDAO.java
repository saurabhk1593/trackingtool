...
@Repository("ticketinfomonthdao")
public class TicketInfoMonthDAO {
...
/*	public TicketInfoDAO(){
	//System.out.println("Inside TicketInfoDAO");
	...
	public LindeMap getTTInfoBySeverity(User userInfo,String month,String year)
	{
		...
		//}
		lindeLogMgr.logMessage("DEBUG","tableObjectName -> "+tableObjectName);
		...
		try {
		   ...
		   //StringBuffer SQL_QUERY = new StringBuffer("select replace(ttinfo.priority,' ',''),count(*) from "+tableObjectName+" ttinfo where ttinfo.status in ('Pending Customer','Work In Progress','Assigned')");
			...
			{
				SQL_QUERY.append("and assigneeFullname in (select UPPER(lastName||', '||firstName) from User where userID='").append(userInfo.getUserID()).append("')");
			}
			else if(userInfo.getRoleId()==2)
			{
				SQL_QUERY.append("and assigneeFullname in (select UPPER(lastName||', '||firstName) from User where location='").append(userInfo.getLocation()).append("')");
			}
...
//chandan method	
	/*public HashMap getSLAdetails(String location) throws Exception,SQLException
    
    {
		
           String pri2query="select count (*) from tbl_cr_info where  priority like '%2%' and (to_date( open_date, 'DD/MM/YYYY HH24:MI:SS' ) - to_date( SYSDATE, 'DD/MM/YYYY HH24:MI:SS' )) > 4 AND phase in ('Accept RfC & Prepare Approval','Validate & Assign') and assignee in(select first_name || ',' || last_name from tbl_users where location='Off-Shore')";
           
           int pri2 =((int)getSession().createQuery(pri2query).uniqueResult());
           lindeLogMgr.logMessage("DEBUG","no of priority 2"+ pri2);
           slaDetailsMap.put("priority2", pri2);
           
           String pri3query="select count (*) from tbl_cr_info where  priority like '%3%' and (to_date( open_date, 'DD/MM/YYYY HH24:MI:SS' ) - to_date( SYSDATE, 'DD/MM/YYYY HH24:MI:SS' )) > 17AND phase in ('Accept RfC & Prepare Approval','Validate & Assign') and assignee in(select first_name || ',' || last_name from tbl_users where location='Off-Shore');";
           int pri3=((int)getSession().createQuery(pri3query).uniqueResult());
           lindeLogMgr.logMessage("DEBUG","no of priority 3"+ pri3);
           slaDetailsMap.put("priority3", pri3);
...
/*		if(year.equals("2008-2009"))
        ...
        try{
        	lindeLogMgr.logMessage("DEBUG", "Before executing status HBL query in status");
             // StringBuffer SQL_QUERY = new StringBuffer(" select replace(ttinfo.status,'Work In Progress','WIP'),app.region,count(*) from "+tableObjectName+" ttinfo,Application app where ttinfo.application=app.applicationname ");
              StringBuffer statusSqlQuery = new StringBuffer("select info1.status, replace(module1.modulename,' ','') ,count(*) from "+tableObjectName+" info1, OwnerModule owner1, Module module1, User user1 where info1.assigneeFullname in (UPPER(user1.lastName||', '||user1.firstName)) and module1.moduleid = owner1.moduleId and user1.userID=owner1.userId and to_date(info1.openTime,'dd/MM/yyyy  HH24:MI:ss') Between ");
              statusSqlQuery.append(" trunc(to_date('"+month+"', 'MON-YYYY'),'MON') AND LAST_DAY(TO_DATE('"+month+"', 'MON-YYYY')) ");
       ...
       try
       {
          ...
          boolean isRowInserted =false;                    
           ...
           for (int i=0;i<query.size();i++) {
               ...
               if(row[0].equals("Pending Customer")) 
               {
                   row[0]="Pending";
             }
               else if(row[0].equals("Closed")) 
               {
                   row[0]="Closed";
             }
	...
	public LindeMap getTTInfoByAge(User userInfo,String month,String year) throws ParseException
	{
		...
		{*/
			tableObjectName="TicketInfo";			
		//}
			  lindeLogMgr.logMessage("DEBUG", "exception in age table ");
				...
				SQL_QUERY.append(" AND CURRENT_DATE BETWEEN (to_date(ttInfo.openTime,'dd/MM/yyyy HH24:MI:ss') + ageGrp.rangeStart) and (to_date(ttInfo.openTime,'dd/MM/yyyy HH24:MI:ss') + ageGrp.rangeEnd) and to_date(ttInfo.openTime,'dd/MM/yyyy  HH24:MI:ss') Between ");
				SQL_QUERY.append(" trunc(to_date('"+month+"', 'MON-YYYY'),'MON') AND LAST_DAY(TO_DATE('"+month+"', 'MON-YYYY')) ");
				if(userInfo.getRoleId()==1)
					SQL_QUERY.append("and ttInfo.assigneeFullname in (").append("select UPPER(lastName||', '||firstName) from User where userID ='").append(userInfo.getUserID()).append("')");
    	...
    	{
    	   Object[] row = (Object[]) it.next();
    	   byAgeMap.put(row[0], row[1]);
    	   total += (Long) row[1];
    	}
    	   byAgeMap.put("Total",total);
    	   lindeLogMgr.logMessage("DEBUG", "Age Details:"+byAgeMap);*/
    	return byAgeMap;	
    }
       ...
       // lindeLogMgr.logMessage("DEBUG", "before try block+++++++++:::::");
        try{
        	//lindeLogMgr.logMessage("DEBUG", "inside try block+++++++++:::::");
             // StringBuffer SQL_QUERY = new StringBuffer(" select replace(ttinfo.status,'Work In Progress','WIP'),app.region,count(*) from "+tableObjectName+" ttinfo,Application app where ttinfo.application=app.applicationname ");
              StringBuffer statusSqlQuery = new StringBuffer("select ageGrp.ageGroup, replace(module1.modulename,' ',''), count(*) from OwnerModule ownermodule1, Module module1, User user1, "+tableBeanName+" ttInfo,TTAge ageGrp where ttInfo.status in ('Pending Customer','Work In Progress','Assigned','Closed') AND CURRENT_DATE BETWEEN (to_date(ttInfo.openTime,'dd/MM/yyyy HH24:MI:ss') + ageGrp.rangeStart) and (to_date(ttInfo.openTime,'dd/MM/yyyy HH24:MI:ss') + ageGrp.rangeEnd) and ttInfo.assigneeFullname in (UPPER(user1.lastName||', '||user1.firstName)) and module1.moduleid=ownermodule1.moduleId and user1.userID=ownermodule1.userId and to_date(ttInfo.openTime,'dd/MM/yyyy  HH24:MI:ss') Between ");
              statusSqlQuery.append(" trunc(to_date('"+month+"', 'MON-YYYY'),'MON') AND LAST_DAY(TO_DATE('"+month+"', 'MON-YYYY')) ");
              if(userInfo.getRoleId()==1)
...
/*	private LindeMap checkAvailability(TicketInfo ttInfo, User user) throws Exception
	...
	*/
	...
	public LindeList getTTDetailedInfoByStatus(String criteria1, String type,String criteria2,User user,String year)
	{
	...
	public LindeList getTTDetailedInfoByAge(String criteria1, String type,String criteria2,User user,String year)
	{
	...
	public LindeList getTTDetailedInfoModulewiseAge(String criteria1, String type,String criteria2,User user,String year)
	{
	...
	public LindeList getTTDetailedInfoBySeverity(String criteria1, String type,String criteria2,User user,String year)
	{
	...
	//	Transaction txn = session.beginTransaction();
		...
		tableObjectName="TicketInfo";
		StringBuffer  SQL_QUERY = new StringBuffer(5000);
		SQL_QUERY.append("SELECT ttinfo.incidentID, ttinfo.title, ttinfo.priority, ttinfo.openTime, ttinfo.status, ttinfo.reopenTime, ttinfo.assigneeFullname, ttinfo.updateTime, ttinfo.closeTime from "+tableObjectName+" ttinfo where status in('Pending Customer','Work In Progress','Assigned') and priority in ('").append(criteria1).append("')");
		if(user.getRoleId()==1)
		{
			SQL_QUERY.append("and ttinfo.assigneeFullname in (").append("select UPPER(lastName||', '||firstName) from User where userID='").append(user.getUserID()).append("')");
		}
...
public String calculateTargetDate(String startDate,int noOfDays) throws ParseException
{
	...
	{
		 lindeLogMgr.logMessage("DEBUG", "inside saveTTinfo method of dao class");
	//	Session session = factory.openSession();
	//	Transaction txn = session.beginTransaction();
		LindeList ttinfo=new LindeList();
		Object[] row;
